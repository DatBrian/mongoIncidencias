...
dotenv.config();
const crearToken = async (req: any, res: any, next: any) => {
    try {
        ...
        const usuario = await conexionDB
            .collection("user")
            .findOne({ user: user });
        ...
        if (!usuario) {
            return res
                .status(401)
                .send({ message: `El usuario ${usuario} no fue encontrado` });
        }

        const passwordMatch = await UserSchema.matchPassword(
            contraseña,
            usuario.contraseña
        );
...
const validarToken = async (req: any, token: any) => {
    try {
        const connection = new Connection();
        await connection.connect();
        const conexionDB = await connection.getDatabase();
        ...
        let res: any = await conexionDB.collection("user").findOne({
            _id: new ObjectId(jwtData.payload.id as string),
